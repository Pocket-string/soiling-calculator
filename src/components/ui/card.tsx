...
export const Card = forwardRef<HTMLDivElement, CardProps>(
  ({ variant = 'default', clickable = false, padding = 'md', className = '', children, ...props }, ref) => {
    return (
      <div
        ref={ref}
        className={`rounded-lg transition-all duration-200 ${variantStyles[variant]} ${paddingStyles[padding]} ${clickable ? 'cursor-pointer hover:shadow-card-hover hover:-translate-y-0.5' : ''} ${className}`}
        {...props}
      >
        {children}
      </div>
